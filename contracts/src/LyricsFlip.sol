...
contract GamePlay is ERC721, Ownable {
    // State variable to store the total number of times the game has been played
    uint256 public totalPlayCount;// Mapping to track the number of plays per wallet address
...
// Constructor to initialize ERC721 and set the deployer as the owner
constructor() ERC721("GamePlayNFT", "GPNFT") Ownable(msg.sender) {
    _tokenIdCounter = 1; // Start token IDs at 1
}

// Function to register a username and associate it with a wallet address
function registerUsername(string calldata _username, address _walletAddress) external {
...
// Function to increment the play count when a game is played
function playGame(string calldata _username) external {
    address playerAddress = usernameToAddress[_username];
    require(playerAddress != address(0), "Username not registered");
    require(msg.sender == playerAddress, "Caller must be the registered wallet");

    totalPlayCount++;
    if (playerPlayCount[playerAddress] == 0) {
        players.push(playerAddress);
    }
    playerPlayCount[playerAddress]++;

    emit GamePlayed(_username, playerAddress, totalPlayCount, playerPlayCount[playerAddress]);
}
